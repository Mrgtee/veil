...
contract VeilShieldTest is Test {
    ...
    function testTransferNoteSpendsNullifierOnce() public {
        bytes32 nullifierHash = keccak256("nullifier-1");

        vm.startPrank(sender);
        shield.transferNote(hex"01", nullifierHash, keccak256("new-note-1"), keccak256("encrypted-new-note"), recipient);
        vm.expectRevert(abi.encodeWithSelector(VeilShield.NullifierAlreadySpent.selector, nullifierHash));
        shield.transferNote(hex"01", nullifierHash, keccak256("new-note-2"), keccak256("encrypted-new-note-2"), recipient);
        vm.stopPrank();
    }
    ...
    function testTransferNoteRejectsInvalidProof() public {
        ...
        shield.transferNote(hex"01", keccak256("nullifier-2"), keccak256("new-note-3"), bytes32(0), recipient);
    }
    ...
    function testWithdrawSpendsNullifierAndTransfersUsdc() public {
        ...
        shield.withdraw(hex"01", nullifierHash, recipient, 4e18);
