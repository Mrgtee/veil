...
5. If `VITE_USE_VEIL_HUB`, `VITE_VEIL_HUB_ADDRESS`, or `VITE_ARC_USDC_ADDRESS` is missing, Veilarc disables submit and shows setup-required details.
6. Veilarc switches/request Arc, reads USDC decimals, balance, and allowance.
7. If allowance is too low, Veilarc asks the wallet to approve VeilHub.
8. Veilarc calls `VeilHub.payOpen`.
9. On transaction success, Veilarc writes a settled API ledger record.
10. If the transaction submits but the API write fails, Veilarc shows the transaction hash and the ledger-write failure.
...
8. Veilarc spends through Circle AppKit with the connected wallet.
9. If spend succeeds and VeilHub is configured, Veilarc registers a VeilHub reference and records `settled`.
10. If spend succeeds and VeilHub is not configured or registration fails, Veilarc records `pending_veilhub_registration`.
11. If balance is deducted but final Arc settlement is delayed, Veilarc records `pending_settlement`.
12. If balance is not deducted, Veilarc does not record success.
...
7. Unified USDC Balance is labeled as `Sequential Unified USDC`: the current Circle AppKit integration spends to one recipient per call, so Veilarc requests one wallet approval/spend per recipient.
8. During sequential Unified USDC Balance payouts, Veilarc shows recipient X of N, awaiting wallet approval, pending settlement, settled, pending VeilHub registration, or failed state for each row.
...
- Veilarc therefore keeps Unified USDC Balance batch honest as a sequential workflow until Circle AppKit exposes a native multi-recipient destination or Veilarc ships a tested escrow distribution contract.

## Unified USDC Balance Deposit

1. User opens `Unified USDC Balance`.
2. User chooses Base Sepolia, Ethereum Sepolia, or Arc Testnet as source.
3. Veilarc requests a wallet network switch.
4. User enters deposit amount.
5. User approves the deposit through Circle AppKit.
6. Veilarc refreshes confirmed and pending balances from the connected wallet.

## Private Payment Selection

1. User can select `Private Payment`.
2. Veilarc shows `Coming soon with Arc Private Kit.`
3. Veilarc explains that it is preparing native Arc privacy integration for hidden/private payment support.
4. Veilarc explains that true private settlement cannot be a visible ERC20 transfer because normal transfers expose amount.
5. Veilarc blocks submit until Arc Private Kit integration is available, wired, tested, and audited.
